...
describe('runResultsImport — succès', () => {
  ...
  test('un provider renvoyant une liste vide (structure rejetée) -> écrit explicitement une liste vide, jamais un simple "rien ne se passe" qui laisserait une ancienne donnée fausse en place', async () => {
    const repo = new FakeResultsRepository({ existingStages: [13] });
    ...
    expect(result.success).toBe(true);
    expect(result.classificationsImported).toBe(4); // general exclu du compte, mais bien écrit vide
    expect(result.warnings.some(w => w.includes('general') && w.includes('non disponible'))).toBe(true);
    const generalWrite = repo.classificationWrites.find(w => w.type === 'general');
    expect(generalWrite).not.toBe(undefined);
    expect(generalWrite.classification.standings).toEqual([]);
  });
