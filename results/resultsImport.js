...
async function runResultsImport({
  ...
  for (const type of Object.values(CLASSIFICATION_TYPES)) {
    ...
    if (!rawEntries || rawEntries.length === 0) {
      ...
      report.warnings.push(w);
      log(`⚠ ${w}`);
      // Écriture explicite d'une liste vide — jamais un simple "continue" :
      // sans ça, une ancienne donnée fausse déjà en base (bug précédent)
      // resterait indéfiniment affichée, même après ce nouvel import.
      await repository.writeClassification(competitionId, stageNumber, type, {
        stageNumber, type, standings: [], lastSynced: startedAt.toISOString()
      });
      continue;
    }
