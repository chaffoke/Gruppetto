...
async function runCompetitionImport({
  ...
  favoritesProvider = null, // optionnel — "Top competitors", jamais bloquant si absent
  teamJerseyProvider = null, // optionnel — maillots d'équipe PCS, jamais bloquant si absent
  resolveRiderId,           // (pcsSlug) => riderId|null, réutilisé pour les favoris
  resolveTeamId,            // (pcsSlug) => teamId|null, réutilisé pour les maillots d'équipe
  repository,
  log = console.log
}) {
  ...
  if (favoritesProvider) {
    try {
      ...
      favorites = assembleFavorites(rawFavorites, { resolveRiderId });
      report.favoritesImported = favorites.length;
    } catch (err) {
      const w = `Favoris PCS introuvables : ${err.message}`;
      report.warnings.push(w);
      log(`⚠ ${w}`);
    }
  }
  ...
  const meta = {
    ...
    favorites, // Competition.meta.favorites[] — une seule liste PCS ("Top competitors"), affichée au niveau compétition/équipes/coureurs, jamais dans la fiche d'une étape précise
  ...
  if (teamJerseyProvider) {
    try {
      ...
      report.teamsImported = teams.length;
    } catch (err) {
      const w = `Maillots d'équipe introuvables : ${err.message}`;
      report.warnings.push(w);
      log(`⚠ ${w}`);
    }
  }
